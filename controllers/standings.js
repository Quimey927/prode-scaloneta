...
module.exports.standings = async (req, res) => {
    ...
    for (let user of users) {
        ...
        if (
            user.results[i] !== null &&
            user.results[i + 1] !== null &&
            prodeOficial.results[i] !== null &&
            prodeOficial.results[i + 1] !== null
        ) {
            let userResult = user.results[i] - user.results[i + 1];
            let prodeOficialResult =
            prodeOficial.results[i] - prodeOficial.results[i + 1];
            if (Math.sign(userResult) === Math.sign(prodeOficialResult)) {
                user.score = user.score + winnerGuess;
                if (
                    user.results[i] === prodeOficial.results[i] &&
                    user.results[i + 1] === prodeOficial.results[i + 1]
                ) {
